...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-time-entry-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) TimeEntryCollection(iD string) {
	timeEntryCollectionData, err := c.callTimeEntrySrvAPIRequirementTimeEntryCollection("TimeEntryCollectionData", iD)
	if err != nil {
		c.log.Error(err)
		return
	}
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": timeEntryCollectionData, "function": "TimeEntryCollectionData"})
	if err != nil {
		c.log.Error(err)
		return
	}
